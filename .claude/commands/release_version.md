...
Perform a release for bevy_restore_window.
...
gh release create "v${VERSION}" \
  --repo natemccoy/bevy_restore_window \
  --title "bevy_restore_window v${VERSION}" \
...
```bash
curl -s "https://crates.io/api/v1/crates/bevy_restore_window" | jq '.crate.max_version'
```
